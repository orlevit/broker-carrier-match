    ...
    def extract_structured_metadata(self, sections: List[Dict[str, Any]], carrier_name: str) -> Dict[str, Any]:
        ...
        for section in sections:
            ...
            if section_level is not None and section_name is not None:
                ...
                for j in range(section_level + 1, 4):
                ...
                if section_level == 1:
                    section_number = str(section_counters[1])
                elif section_level == 2:
                    section_number = f"{section_counters[1]}.{section_counters[2]}"
                elif section_level == 3:
                    section_number = f"{section_counters[1]}.{section_counters[2]}.{section_counters[3]}"
...
if __name__ == "__main__":
    ...
    for i, result in enumerate(results):
        ...
        # Print structured metadata if present
        if "structured_metadata" in result:
            if "Natural_disaster" in result["structured_metadata"]:
                print("Natural Disaster Coverage:")
                print(f"  Include: {', '.join(result['structured_metadata']['Natural_disaster']['include'])}")
                print(f"  Exclude: {', '.join(result['structured_metadata']['Natural_disaster']['exclude'])}")
        
        # Print truncated content
        print("\nContent:")
        print(result['text'][:200] + "..." if len(result['text']) > 200 else result['text'])
    
    # Example metadata-based query
    print("\n=== Metadata-Based Query ===")
    metadata_results = app_db.query_by_metadata(
        region="California",
        natural_disaster="Wildfire",
        include_exclude="include"
    )
    
    print(f"Found {len(metadata_results)} carriers covering wildfire risks in California:")
    for result in metadata_results:
        print(f"- {result['carrier_name']}")
