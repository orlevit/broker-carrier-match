...
class AppetiteGuideVectorDB:
    ...
    def build_section_hierarchy(self, sections: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        """
        Build a hierarchical structure of sections with proper parent-child relationships
        
        Args:
            sections: List of parsed markdown sections
            
        Returns:
            Dictionary mapping section titles to information including their parents
        """
        section_hierarchy = {}
        current_headers = [None] * self.max_header_depth
        
        for section in sections:
            # Determine the current section level and title
            section_level = None
            section_title = None
            ...
            for i in range(1, self.max_header_depth + 1):
                ...
                if header_key in section.metadata:
                    section_level = i - 1  # Zero-based index
                    section_title = section.metadata[header_key]
                    break
            
            if section_level is not None and section_title is not None:
                # Update the current headers at this level
                current_headers[section_level] = section_title
                
                # Clear all headers below this level
                for i in range(section_level + 1, self.max_header_depth):
                    current_headers[i] = None
                
                # Collect parent headers (all non-null headers above this level)
                parents = [current_headers[i] for i in range(section_level) if current_headers[i] is not None]
    ...
    def extract_metadata_with_openai(self, text: str, carrier_name: str, 
                                     content_type: str = "guide", 
                                     parents: List[str] = None,
                                     section_hierarchy: Dict[str, Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            parents: List of parent section titles
            section_hierarchy: Dictionary of section hierarchy information
        ...
        parents_info = ""
        if parents:
            parents_info = f"\nParent sections: {', '.join(parents)}"
        
        # Prepare section hierarchy info for the prompt
        hierarchy_info = ""
        if section_hierarchy:
            hierarchy_info = "\nSection hierarchy information:\n"
            for section_title, info in section_hierarchy.items():
                parent_list = ", ".join(info["parents"]) if info["parents"] else "None"
                hierarchy_info += f"- '{section_title}' has parents: {parent_list}\n"
...
Carrier Name: {carrier_name}{parents_info}{hierarchy_info}
...
"parents": {json.dumps(parents) if parents else "null"},
"geographical_region": {{
...
- The "parents" field should contain the list of parent section titles as provided
...
"""
        ...
        try:
            ...
            try:
                ...
                # Ensure parents is set correctly
    ...
    def create_hierarchical_chunks(self, sections: List[Dict[str, Any]], 
        ...
        Each chunk will have its own metadata extracted using OpenAI with proper parent information
        ...
        section_hierarchy = self.build_section_hierarchy(sections)
        
        # Extract metadata for the full guide
        full_guide_metadata = self.extract_metadata_with_openai(
            full_guide, carrier_name, content_type="guide", 
        ...
        for section in sections:
            ...
            if section_title and section_title in section_hierarchy:
                # Get parent information
                parents = section_hierarchy[section_title]["parents"]
                ...
                section_metadata = self.extract_metadata_with_openai(
                    ...
                    parents=parents, section_hierarchy=section_hierarchy
                )
                ...
                for bullet in bullet_points:
                    ...
                    # For bullet points, add the current section to the parents list
                    bullet_parents = parents + [section_title] if parents else [section_title]
                    ...
                    bullet_metadata = self.extract_metadata_with_openai(
                        ...
                        parents=bullet_parents, section_hierarchy=section_hierarchy
                    )
...
if __name__ == "__main__":
    ...
    for i, result in enumerate(results):
        ...
        # Print parent sections if present
        if result['metadata'].get('parents'):
